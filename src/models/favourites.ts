...
export class Favourites {
    public static async findOneByObjectID(userId: number): Promise<IFavourite | null> {
    ...
    public static async findAllByObjectId(objectId: number): Promise<IFavourite[] | null> {
    ...
    public static async findOneByUserID(userId: number): Promise<IFavourite | null> {
        const query = 'SELECT * FROM favourites WHERE user_id = ? LIMIT 1;';
        const result: [any, FieldPacket[]] | QueryError = await MySQL.execute(query, [userId]);
        return returnSingle(result);
    }

    public static async findAllByUserID(userId: number): Promise<IFavourite[] | null> {
